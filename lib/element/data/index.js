...
function defBind () {
  return (this._contextLevel === 1 ? (this._context || this._parent)  : this._parent).parseValue()
}

// function walkcntx (parent§) {
...
function getit (field) {
  ...
  return function fieldbind () {
    ...
    var parent = this._contextLevel === 1 ? (this._context || this._parent) : this._parent
    while (parent) {
      self = parent._input
      if (self) {
        let a = self.retrieve && self.retrieve(field)
        // console.error(field, '--->', self.path, a)
        return a ? a.parseValue() : 'smurky:' + field
      }
      parent = parent._contextLevel === 1 ? parent._context : parent._parent //_context if context else stop
    }
    return ''
  }
}
  ...
  define: {
    ...
    origin: {
      get () {
        ...
        var field = this.$
        var reference = this
        console.log('yo!', field, reference)
        if (field) {
          let self
          let parent = this._contextLevel === 1 ? (this._context || this._parent) : this._parent
          console.log('/??????', parent, this._contextLevel, this._context)
          while (parent) {
            ...
            parent = parent._contextLevel === 1 ? (parent._context || this._parent) : parent._parent
          }
        } else {
...
exports.define = {
  ...
  $subscribeProperty (p, target) {
    ...
    if (p.$) {
      ...
      let subscontext = cntxt ? store[store.length - 1] : element
      console.log('yo', p.$)
