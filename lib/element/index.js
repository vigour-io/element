  ...
  define: {
    addNewProperty (key, val) {
      ...
      if (val && val._datarender) {
        console.info('ok ok ok!', this._path, key)
        var p = this
    ...
    render (data, l) {
      ...
      // accept state then its easy to cal lit directly
      var dstamp = this._datarender && data && data._lstamp // this is wrong way too many now...
      // (this.$ || this._datarender) &&
...
function comparestamps (previousState, currentState) {
  console.info('compare:', previousState.stamp, 'vs', currentState.stamp, currentState.elem.path)
  var res =
    currentState.force ||
    previousState.stamp !== currentState.stamp ||
    currentState.l !== previousState.l ||
    currentState.data !== previousState.data // also not correct...
...
function genericRender (data, current, prev) {
  ...
  if (data && this.$collection && data.keys()) {
      ...
      if (this.keys()) {
        for (let i = 0, len = this._keys.length; i < len; i++) {
          if (data[this._keys[i]] === void 0) {
            // when remove its nice to not add it event if it was a child
            children.push(this[this._keys[i]].render(data))
          }
        }
      }
    }
    ...
    if (this.keys()) {
      for (let i = 0, len = this._keys.length; i < len; i++) {
        children.push(this[this._keys[i]].render(data, len))
      }
    }
  }
