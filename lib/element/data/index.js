...
exports.define = {
  ...
  renderData (target, oldVal, event, old$, map) {
    // console.log('yo renderData', target.path)
    var element = this
    if (!map) {
      map = this.$map()
    }
    // console.log('yo renderData', target.path, map)
      ...
      if (obj.$context) {
        ...
        if (element.listensOnAttach && element._input !== target) {
          element.listensOnAttach.each(function (p, i) {
            ...
            if (!keep) {
              console.error('REMOVE AAAA', i, element.listensOnAttach[i].attach.keys())
          ...
          if (obj.$collection) {
            target.subscribe(obj.$, 'property', [
              function (data, event) {
                ...
                if (origin._input === null) {
                  ...
                  element._input = origin
                  // dont put function on the bottom! (breaks on tv in strict mode)
            ...
            target.subscribe(obj.$, 'data', [
              function thisone (data, event) {
                var origin = this.origin
                // console.error('target!', element.path)
                var rerender
