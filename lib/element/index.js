...
function genericRender (data, current, prev) {
  ...
  if (data && this.$collection && data._speshkeys) {
    for (var i = 0, len = data._speshkeys.length; i < len; i++) {
      var key = data._speshkeys[i]
  ...
  if (this.children) {
    ...
    for (var i = 0, len = this.children.length; i < len; i++) {
      children.push(this[this.children[i]].render(data, props, children))
    }
  }

  if (this.props) {
    for (var i = 0, len = this.props.length; i < len; i++) {
      loopProperty(this[this.props[i]], data, props, children, current, prev)
    }
  }
...
function loopProperty (property, data, props, children, current, prev) {
  if (property.render) {
    ...
    if (property.props) {
      for (var i = 0, len = property.props.length; i < len; i++) {
        loopProperty(property[property.props[i]], data, props, children, current, prev)
      }
    }
  }
}
