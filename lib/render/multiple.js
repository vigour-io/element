...
module.exports = function renderMultiple (state, type, stamp, subs, tree, sType) {
  ...
  if (_) {
    if (sType === 'done') {
      if (_.d) {
        console.error('DONE IS NOT SUPPORTED (TEMP)')
        // for (let i = 0, len = _.da.length; i < len; i += 3) {
          // render(_.da[i + 1], state, type, stamp, subs, tree, _.da[i])
        // }
      }
    } else {
      if (type !== 'update' && _.ta) {
        for (let i = 0, len = _.ta.length; i < len; i += 3) {
          render(_.ta[i + 2], state, type, stamp, subs, tree, _.ta[i], _.ta[i + 1])
        }
      }
      if (_.sa) {
        for (let i = 0, len = _.sa.length; i < len; i += 3) {
          render(_.sa[i + 2], state, type, stamp, subs, tree, _.sa[i], _.sa[i + 1])
        }
      }
    }
  }
