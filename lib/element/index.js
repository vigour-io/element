...
function comparestamps (previousState, currentState) {
  ...
  if (!res) {
    console.warn('ze fuck is wrong?', currentState.elem.path, previousState.stamp, currentState.stamp)
  }
  return res
}
...
function genericRender (data, current, prev) {
  ...
  var children = []
  var props = current.state && current.state.props && current.state.props.properties || { key: this.key }
  if (this.$put) {
    var check
    if (prev) {
