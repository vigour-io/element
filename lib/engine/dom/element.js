      ...
      if (this.engine) { // engine needs to be sort of the same as context
        ...
        if (val instanceof Element) {
          ...
          if (node && !this.engine.nodes[val.getHashedPath()]) {
            if (val.$ && !this.$collection) {
              console.log('!!!!!!!!!')
              let parent = this
              while (parent) {
                if (parent.storedmap) {
                  delete parent.storedmap
                }
                // if (parent.) is the origin -- this is important and hard
                // find it...
                parent = parent.parent
              }
            }
            // // if (this.$ && !(this.ChildConstructor !== Element && val instanceof this.ChildConstructor)) {
        ...
        // if (!this.$collection && this.$ && !(val instanceof this.ChildConstructor)) {
        //   console.log('yo yo yo')
        //   var parent = this
        //   while (parent) {
        //     // console.log('???')
