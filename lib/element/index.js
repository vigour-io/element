...
var Element = module.exports = new Observable({
  ...
  define: {
    ...
    render (data, state, event, map) {
      ...
      console.error('??????', this.path, data && data.path)
      ...
      console.error('yo render!', 'data:', data, 'path:', this.path)
...
function genericRender (data, current, prev) {
        ...
        for (var i = 0, len = data._keys.length; i < len; i++) {
          ...
          if (this[key] === null) {
            ...
            if (this.properties[type].base && this.properties[type].base.render) {
              ...
              console.error('!#!@!FUCK@#!@#', this.properties[type].base, property)
              ...
              console.error('----->', this.properties[type].base)
  ...
  // add key
  if (props.className) {
    props.className = this.key + ' ' + props.className
  } else {
    props.className = this.key
  }
