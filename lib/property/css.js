          ...
          $change: function(event, removed) {
            ...
            if (length > 1) {
              for (i = 0; i < length; i = i + 1) {
                ...
                if (index === -1) {
                  $cssVal = ($cssVal + ' ' + valArr[i]).replace('  ', ' ')
                }
              }
      ...
      $removeClass: new Property({
        $on: {
          $change: function(event, removed) {
            ...
            if (length > 1) {
              for (i = 0; i < length; i = i + 1) {
                $cssVal = $cssVal.replace(cssArr[i], '')
              }

              $css.$val = $cssVal/replace('  ', ' ')
            }
            else {
              index = $cssVal.indexOf($val)

              if (index !== -1) {
                $css.$val = $cssVal.replace($val, '').replace('  ', ' ')
              }
            }
