  ...
  define: {
    ...
    render (data) {
      ...
      state = {
        ...
        dataid: data && data.uid // temp hack for dom events on nested fields -- never know if they use the data
        // dataid: (this._datarender && data && data._input !== null && data.uid) || false // this._datarender &&
      }
...
function comparestamps (previousState, currentState) {
  ...
  console.info(currentState.elem.path, currentState.dataid, previousState.dataid)
        ...
        for (var i = 0, len = data._keys.length; i < len; i++) {
          ...
          if (this[key] && this[key] instanceof Element) {
            ...
            if (this.properties[key].prototype && this.properties[key].prototype.render) {
              ...
              let ret = this.properties[key].prototype.render(property, props, children, data)
              if (ret) { children.push(ret) }
            }
          } else {
            // better to put it straight in state of course make your context
