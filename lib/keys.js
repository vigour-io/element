...
exports.define = {
  ...
  //     if (ordered) {
  //       // @todo: .sort is very slow fix this using a custom qsort
  //       keys.sort(state ? this.stateSort.bind(this, state) : this.sort.bind(this))
  //     } else if (easyorder) {
  //       keys.sort((a, b) => this[a].__order - this[b].__order)
  //     } if (!keys[0]) {
  //       keys = this[field] = false
  ...
  //   let bi = this[b].order || 0
  //   let stateval
  //   ai = ai.$ && ai.compute && ai.compute(stateval = state.val) || ai || 0
  //   bi = bi.$ && bi.compute && bi.compute(stateval || state.val) || bi || 0
  //   return ai === bi ? this[a].__order - this[b].__order : ai < bi ? -1 : 1
  // }
}
