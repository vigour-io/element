...
var Element = module.exports = new Observable({
  ...
  define: {
    ...
    render (data) {
      // need event here can overwrite blocks
      data = getData(this, data)
      //pass stamp then we can at least compare that -- or data stamp / normal stamp
      console.log('got data prefer over old')
      var state = {
        // lets make this d stamp and lstamp
        lstamp: this._lstamp,
...
function comparestamps (previousState, currentState) {
  ...
  var res =
    ...
    // this is a hack should not be the thing!
    // currentState.elem.uid !== previousState.elem.uid // REMOVE THIS SOMETHING IN SET EXT

  if (!res && !currentState.props) {
    currentState.props = previousState.props // need to be sure stuff gets removed
  }
  return res
}
