...
module.exports = {
  ...
  src: {
    ...
    render (node, event, player) {
      ...
      console.error('wut src---->', src, instance)
      if (src) {
        if (!instance) {
          init(player, node)
        } else {
          ...
          console.error('AM I GOING TO PLAY ?? ', player.play.origin.val, this.plain())
          instance
            .load(this.plain())
            .catch((err) => {
              player.play.origin.val = false
...
function config (player, node) {
  ...
  console.error('set source:', player.src.plain())
  ...
  bitplayer.setup(config).then(function (value) {
    console.error('set config:', value)
    ...
    console.log('CARALHA!', player.src.val)
    instance.addEventHandler('onReady', () => {
      ...
      player.applyContext(store)
      console.log('I AM GOING TO PLAY!!!', player.play.origin.syncPath, player.play.origin)
      // player.play.origin.val = false
      player.play.origin.val = true
    })
    if (hasBuiltinAd) {
