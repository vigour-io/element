...
var Element = module.exports = new Observable({
  ...
  properties: {
    widget (obj) {
      ...
      // previous, domNode
      this.widget.prototype.update = obj.update
      this.widget.prototype.init = obj.init
      this.widget.prototype.destroy = obj.destroy
      this.widget.prototype.type = 'Widget'
    },
    renderNode: true,
    renderTree: true,
    state: true,
  ...
  define: {
    vdom: {
      value: {
        ...
        diff: diff,
        patch: patch,
        applyPatch: require('virtual-dom/vdom/patch-op.js')
      }
    },
    keysCheck (val, key) {
    ...
    forceUpdates () {
      console.warn('calling force updates probably spawned from parent subscription OR FROM FIX FOR CASES', this.path.join('.'))
      var p = this
      while (p && !p._force) {
...
function put (property, key, children) {
  var type = key
  if (this[key] === null) {
    console.warn('removed an element (and resolved context) in a $put are you sure?')
