...
var Element = module.exports = new Observable({
  ...
  properties: {
    ...
    type (val, event) {
      if (this.type !== val) {
        this.node = val
        this.type = val // may need this or something...
      }
    },
    DOM (DOMnode) {
  ...
  define: {
    ...
    render (data, state, event, prev) {
      ...
      if (this.$ && ((data && data.__input === null || !data))) {
        return
      }
...
Element.Thunk = Thunk
Element.isProp = isProp
Element.addChild = addChild
Element.loopProperty = loopProperty
...
function genericRender (data, current, prev) {
  ...
  // props.style && console.info('props:', JSON.stringify(props.style, false, 2))
      ...
      if (data.keys()) {
        for (let i = 0, len = data._keys.length; i < len; i++) {
          ...
          if (!this._properties[key] || (
            ...
            this._properties[key].base &&
            this._properties[key].base instanceof Element)
          ) {
