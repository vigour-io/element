...
var Element = module.exports = new Observable({
  ...
  define: {
    ...
    render (data, state, event, prev) {
      ...
      if (this.path.join('.') === 'app.main.switcher.2039') {
        console.info(force, this._force , !dstamp && this._datarender , this._forceUpdates)
      }
...
function comparestamps (previousState, currentState) {
  ...
  if(currentState.elem.path.join('.') === 'app.main.switcher.2039') {
    console.log('++++++++>')
    console.error( currentState.force, '|',
    previousState.stamp , currentState.stamp, '|',
    currentState.elemid, previousState.elemid, '|',
    currentState.dataid, previousState.dataid, currentState.elem.path.join('.'))
    console.log('++++++++>')
  }
