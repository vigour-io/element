...
function getit (field) {
  return function fieldbind () {
    console.log('be renderin', this.path)
    var self
    var parent = this
    while (parent) {
      if (self) {
  ...
  element.each(function (prop, key) {
    ...
    if (prop.$) {
      if (prop.$ === true) {
        ...
        if (!mytarget) {
          nothin = true
          // console.error('now have to do shit', prop._path) // this is allways nessecary switching refs etc just execute it when its there
          // this is ncie since you unfify colleciton /w normal $
