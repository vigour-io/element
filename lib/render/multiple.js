...
module.exports = function renderMultiple (state, type, stamp, subs, tree, sType) {
  const _ = subs._
  // if(sType) {
  //   console.log('%c'+type+' - '+sType, 'color: red', state.path(), tree)
  // } else {
  //   console.log('%c'+type, 'color: red', state && state.path(), tree)
  // }
  if (sType === 'done') {
    if (_.dList) {
    ...
    if (type === 'remove-ref') {
      console.error('%cREMOVE:', 'background:black;color:white',_)
    }
    if (_.tList) {
