...
exports.define = {
  renderData (target, oldVal, event, old$, map, mapwalker) {
    var element = this
    if (!map) {
      if (mapwalker) {
        map = this.$map(void 0, void 0, mapwalker)
      } else {
        map = this.$map()
      }
    }
        ...
        if (element.listensOnAttach && element._input !== target) {
          element.listensOnAttach.each(function (p, i) {
            ...
            if (!keep) {
              ...
              element.listensOnAttach[i].off('attach', i)
              // element.listensOnAttach[i].remove()
              // delete element.listensOnAttach[i]
            }
          })
        }
            ...
            target.subscribe(obj.$, 'property', [
              function (data, event) {
                ...
                if (origin._input === null) {
                  ...
                  origin.each(iterator)
                  function iterator (property, key) {
                    if (!property) {
                      return
                    }
