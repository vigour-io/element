...
exports.define = {
  getType (val, event, key, nocontext, escape) {
    ...
    console.info('lets start get info:', key)
    let result = lookUpComponent(this, type, val)
    if (result) {
      val.node = result.node
      console.log('yo result!', type, val, result)
      console.log('now the set should result in todos!')
...
exports.properties = {
  Child (val, event) {
    console.error('----> set c-csntr', val)
    if (isPlain(val) && val.type) {
      console.log('lets go get dat type')
