...
var render
if (!isNode) {
  render = require('../../engine/dom/render')
}
...
exports.define = {
  ...
  $subscribeProperty (p, target) {
    ...
    if (p.$) {
      ...
      }, subscontext ], subscontext.uid || element)
    }
  },
  renderData (target, oldVal, event, old$, map) {
    ...
    function walk (obj, element, target, rerenderinherit) {
      ...
      if (obj.$context) {
        // if(rerenderinherit) {
        //   console.log('prop have to clear or something', element.path, 'X', element._context._path)
        // }
        // console.warn('------>', 'hello!!! cntxt')
        for (let i in obj) {
              ...
              function (data, event) {
                ...
                if (origin._input === null) {
                  ...
                  origin.each(function (property, key) {
                    ...
                    if ((!elem || elem.input !== origin) && origin._input !== null) {
                      // console.log('lettttz make!', key, origin._input)
                      // if second lets unsubscribe everything
                      // all listeners in attach on the target!
                      // make a function for this -- removeAllAttach(Element) // this way handle on data works for elements and collos
            ...
            target.subscribe(obj.$, 'data', [
              function thisone (data, event) {
                ...
                if (element._input && this.origin !== element._input || (stamp === event.stamp && oldVal)) {
                  ...
                  console.log('SWITCH')
                  if (origin && origin._input !== null && !isNode) {
                ...
                for (let i in obj) {
                  if (element[i] && element[i] instanceof Prop) {
                    ...
                    walk(obj[i], element[i], origin, rerender)
                  }
                }
                if (rerender && !rerenderinherit) {
                  console.error('!!!rerender!!!') // only do for top!
