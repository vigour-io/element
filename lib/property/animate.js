...
function animate (event, $settings) {
  ...
  var speed = {
    x: ($settings.$x - current.x) / duration,
    y: ($settings.$y - current.y) / duration,
    opacity: ($settings.$opacity - current.opacity) / duration
  }
  ...
  var moveAction = function (timestamp) {
    ...
    if (progressTime < duration) {
      moveAnim = window.requestAnimationFrame( moveAction );
    } else {
      if( callback ) {
        callback.call( $element, event )
      }
