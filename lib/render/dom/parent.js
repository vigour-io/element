...
module.exports = function (target, state, type, stamp, subs, tree, ptree, pnode) {
  if (!pnode) {
    ...
    if (parent) {
      ...
      if (_.s && _.s[parentUid] || _.t && _.t[parentUid]) {
        pnode = tree._[parentUid] || render('dom', parent, state, type, stamp, subs, tree, ptree, parentUid)
      } else {
