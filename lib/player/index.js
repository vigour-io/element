...
function render (node, event, player) {
  let exec = this.exec
  if (exec) {
    console.log('kanker:', this.key, this.val, 'ready:', player.ready.val === true)
...
var VideoProperty = new Property({
  ...
  define: {
    ...
    set (val, event) {
      let type = typeof val
      console.error('set this!', this.key, val, type)
      if (!val || type === 'boolean' || type === 'number' || type === 'string') {
        if (val !== this._input) {
          this._input = val
          if (event === void 0) {
...
var Player = new Element({
  ...
  play: {
    val: false,
    exec (node, e, player) {
      console.log('play:', this.val)
      if (this.val) {
        console.log('interval:', player._interval)
        if (!player._interval) {
          let store = player.storeContext()
