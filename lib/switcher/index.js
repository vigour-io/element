    ...
    if (length > 1) {
      ...
      if (transition && this.state) {
        ...
        for (let i = length - 2; i >= 0; i--) {
          let props = childprops(children[i])
          props.style.position = 'absolute'
          transitionRender(transition, props)
          transformRender[axis](-delta * direction, props)
        }
        ...
        let props = childprops(lastchild)
        transformRender[axis](delta * direction, props)
        ...
        transitionRender(transition, state.next.props.properties)
        transformRender[axis](0, state.next.props.properties)

        // this.postpone('switch', function () {
        //   for (var i in current.vnode.children) {
        //     current.vnode.children[i].state = null
        //   }
