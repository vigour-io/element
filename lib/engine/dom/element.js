  ...
  exports.define = {
    ...
    addNewProperty (key, val, property, event) {
      ...
      if (this.engine) { // engine needs to be sort of the same as context
        ...
        // CLEAN UP not often enough needs sort of $root system
        ...
        // console.warn('clear map to be sure -- too heavy should only be called before renders (for now)', val._path)
      }
      // do the checks
      // if (val instanceof Element) {
      // console.log(val._root$)
      // checks for collection etc
      return ret
    }
  }
