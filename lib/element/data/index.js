          ...
          if (obj.$collection) {
            ...
            target.subscribe(obj.$, 'data', [
              function thisone (data, event) {
                console.log('change listener AGAIN!')
                var origin = this.origin
                var rerender
                if (element._input && this.origin !== element._input) {
                  if (element.listensOnAttach) {
                    element.listensOnAttach.each(function (p, i) {
                      var keep
                      // to much eachg attach keyts will become uids!!!!!
                      for (var j in p.attach) {
                        if (p.attach[j][1] === element) {
                          if(p.attach[j][0] === thisone) {
                            keep = true
                            console.error('!#@@!#@!#!@#!@#!@#!@#@!#')
                          } else {
                            delete element.listensOnAttach[i].attach[j]
                          }
                        }
                      }
                      // have to handle collection as well!
                      if(!keep) {
                        element.listensOnAttach[i] = null
                      }
                    })
                  }
