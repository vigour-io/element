...
function getit (field) {
  ...
  return function fieldbind () {
    ...
    while (parent) {
      self = parent._input
      if (self) {
        // field === 'mrss' && console.error(field, self.path)
...
exports.define = {
  ...
  $subscribeProperty (p, target) {
    ...
    if (p.$) {
      ...
      // let store = element.storeContext()

      let subscontext = cntxt || element
      // element._marked = true
      if (p.$ === 'time') {
        console.log('SUBSCRIBE PROP!!!', p.$, p._path, target._path)
      }
      target.subscribe(p.$, 'data', [ function (data, event) {
        // Element
        // console.log('bitch plz!', this.path, p.$, p._path)
        if (cntxt) {
          ...
          if (obj.$collection) {
            ...
            target.subscribe(obj.$, 'data', [
              function thisone (data, event) {
                console.log('change listener AGAIN!', obj.$, element._path)
                var origin = this.origin
